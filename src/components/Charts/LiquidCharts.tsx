...
const SpectrumTooltip = ({ active, payload, total }: any) => {
    if (active && payload && payload.length) {
        ...
        return (
            ...
            >
                <p style={{ margin: "0 0 8px 0", fontSize: "14px", fontWeight: 600, color: "rgba(255,255,255,0.7)", borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: "4px" }}>
                    Proof Breakdown
                </p>
                {payload.map((entry: any, index: number) => {
                    ...
                    return (
                        <div key={index} style={{ marginBottom: "8px" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", color: entry.color, fontWeight: 700, fontSize: "15px" }}>
        ...
        <div style={{ width: "100%", height, position: "relative" }}>
            <ResponsiveContainer>
                ...
                >
                    ...
                    <Tooltip cursor={{ fill: 'transparent' }} content={<SpectrumTooltip total={maxValue} />} />
                    ...
                    {data.map((category, index) => {
                        ...
                        const radius: [number, number, number, number] =
                            data.length === 1 ? [12, 12, 12, 12] :
                                isFirst ? [12, 0, 0, 12] : // Top-Left, Top-Right, Bottom-Right, Bottom-Left... wait, layout vertical?
                                    ...
                                    isFirst ? [12, 0, 0, 12] : // Left side rounded
                                        isLast ? [0, 12, 12, 0] : // Right side rounded
                                            [0, 0, 0, 0];
        ...
        return (
            ...
            >
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                    <span style={{ color: "rgba(255,255,255,0.7)", paddingRight: "4px" }}>Collection Value: </span>
                ...
                {/* Count (Middle) */}
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                    <span style={{ color: "rgba(255,255,255,0.7)" }}>Total Bottles: </span>
...
export const LiquidDistributionChart: React.FC<LiquidDistributionChartProps> = ({
    data,
    height = 300,
}) => {
    return (
                ...
                <PieChart>
                    ...
                    >
                        {data.map((entry, index) => (
                            <Cell key={`cell-value-${index}`} fill={entry.color} opacity={1} />
                        ))}
                    </Pie>
                    ...
                    >
                        {data.map((entry, index) => (
                            <Cell key={`cell-count-${index}`} fill={entry.color} opacity={0.6} />
                        ))}
                    </Pie>
